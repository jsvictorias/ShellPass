...
export default function Question7() {
  ...
  const question8DocRef = doc(collection(db, 'perguntas'), 'Question8');

  const handleOptionSelect1 = async (option) => {
    setSelectedOption(option);
    setIsNextButtonEnabled(true);
    if (option === 'Mínimo') {
      await updateDoc((question8DocRef), {
        resposta1: true,
        resposta2: false
      })
    }
    ...
    <View style={styles.container}>
      <View style={styles.Body}>
        <View style={styles.Corpo}>
          <BotaoVolta1 />
          <Text style={{ left: '3%', fontSize: 17, fontWeight: 'bold', top: '10%' }}>
            O líquido de arrefecimento é um pequeno tanque de plástico transparente com marcações de nível, olhe no reservatório o seu nível.
          </Text>
        </View>
