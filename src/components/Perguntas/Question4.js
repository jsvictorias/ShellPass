...
export default function Question4() {
  ...
  const question5DocRef = doc(collection(db, 'perguntas'), 'Question5');

  const handleOptionSelect1 = async (option) => {
    setSelectedOption(option);
    setIsNextButtonEnabled(true);
    if (option === '1 - 3 semanas atrás') {
      await updateDoc((question5DocRef), {
        resposta1: true,
        resposta2: false,
        resposta3: false,
        resposta4: false,
    ...
    <View style={styles.container}>
      <View style={styles.Body}>
        ...
        <View style={styles.phoneFrame}>
          <Text style={styles.title}>Qual a última vez que trocou os pneus?   </Text>
