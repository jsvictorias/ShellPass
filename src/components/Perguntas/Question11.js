...
export default function Question11() {
  ...
  const question12DocRef = doc(collection(db, 'perguntas'), 'Question12');

  const handleOptionSelect1 = async (option) => {
    setSelectedOption(option);
    setIsNextButtonEnabled(true);
    if (option === 'Menos de 6 meses') {
      await updateDoc((question12DocRef), {
        resposta1: true,
        resposta2: false,
        resposta3: false,
        resposta4: false,
    ...
    <View style={styles.container}>
      <View style={styles.Body}>
        ...
        <View style={styles.phoneFrame}>
          <Text style={styles.title}>Última troca de mangueiras e correias?</Text>
