...
export default function Question5() {
  ...
  const question6DocRef = doc(collection(db, 'perguntas'), 'Question6');

  const handleOptionSelect1 = async (option) => {
    setSelectedOption(option);
    setIsNextButtonEnabled(true);
    if (option === '1 - 3 semanas atrás') {
      await updateDoc((question6DocRef), {
        resposta1: true,
        resposta2: false,
        resposta3: false,
        resposta4: false,
    ...
    <View style={styles.container}>
      <View style={styles.Body}>
        ...
        <View style={styles.phoneFrame}>
          <Text style={styles.title}>Qual a última vez que trocou o filtro de ar?   </Text>
