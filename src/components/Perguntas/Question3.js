...
export default function Question3() {
  ...
  const question4DocRef = doc(collection(db, 'perguntas'), 'Question4');

  const handleOptionSelect1 = async (option) => {
    setSelectedOption(option);
    setIsNextButtonEnabled(true);
    if (option === '1 - 3 semanas atrás') {
      await updateDoc((question4DocRef), {
        resposta1: true,
        resposta2: false,
        resposta3: false,
        resposta4: false,
    ...
    <View style={styles.container}>
      <View style={styles.Body}>
        ...
        <View style={styles.phoneFrame}>
          <Text style={styles.title}>Qual a última vez que trocou o óleo?  </Text>
