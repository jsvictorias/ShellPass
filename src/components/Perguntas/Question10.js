...
export default function Question10() {
  ...
  const question11DocRef = doc(collection(db, 'perguntas'), 'Question11');

  const handleOptionSelect1 = async (option) => {
    setSelectedOption(option);
    setIsNextButtonEnabled(true);
    if (option === 'Mínimo') {
      await updateDoc((question11DocRef), {
        resposta1: true,
        resposta2: false
      })
    }
    ...
    <View style={styles.container}>
      <View style={styles.Body}>
        ...
        <View style={styles.phoneFrame}>
          <Text style={styles.title}>Qual o nível do fluído do líquido de transmissão?</Text>
