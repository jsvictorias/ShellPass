...
export default function Question15() {
  ...
  const question16DocRef = doc(collection(db, 'perguntas'), 'Question16');

  const handleOptionSelect1 = async (option) => {
    setSelectedOption(option);
    setIsNextButtonEnabled(true);
    if (option === 'Menos de 6 meses') {
      await updateDoc((question16DocRef), {
        resposta1: true,
        resposta2: false,
        resposta3: false,
        resposta4: false,
    ...
    <View style={styles.container}>
      <View style={styles.Body}>
        ...
        <View style={styles.phoneFrame}>
          <Text style={styles.title}>Qual a última troca dos cabos de ignição e velas?</Text>
